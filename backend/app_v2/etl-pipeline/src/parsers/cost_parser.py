...
class SpecificationParser:
    ...
    SPEC_PATTERNS = {
        'capacity': [
            r'capacity[:\s]+(\d+(?:,\d{3})*(?:\.\d+)?)\s*(' + UNIT_PATTERNS['capacity'] + ')',
            r'throughput[:\s]+(\d+(?:,\d{3})*(?:\.\d+)?)\s*(' + UNIT_PATTERNS['capacity'] + ')',
            r'rated\s*at\s*(\d+(?:,\d{3})*(?:\.\d+)?)\s*(' + UNIT_PATTERNS['capacity'] + ')',
        ],
        'power': [
            r'power[:\s]+(\d+(?:,\d{3})*(?:\.\d+)?)\s*(' + UNIT_PATTERNS['power'] + ')',
            r'motor[:\s]+(\d+(?:,\d{3})*(?:\.\d+)?)\s*(' + UNIT_PATTERNS['power'] + ')',
            r'installed\s*power[:\s]+(\d+(?:,\d{3})*(?:\.\d+)?)\s*(' + UNIT_PATTERNS['power'] + ')',
        ],
        'diameter': [
        ...
        'weight': [
            r'weight[:\s]+(\d+(?:,\d{3})*(?:\.\d+)?)\s*(' + UNIT_PATTERNS['weight'] + ')',
            r'mass[:\s]+(\d+(?:,\d{3})*(?:\.\d+)?)\s*(' + UNIT_PATTERNS['weight'] + ')',
        ],
        'speed': [
            r'speed[:\s]+(\d+(?:\.\d+)?)\s*(' + UNIT_PATTERNS['speed'] + ')',
            r'operating\s*speed[:\s]+(\d+(?:\.\d+)?)\s*(' + UNIT_PATTERNS['speed'] + ')',
        ],
        'css': [
            r'css[:\s]+(\d+(?:\.\d+)?)\s*(' + UNIT_PATTERNS['length'] + ')',
            r'closed\s*side\s*setting[:\s]+(\d+(?:\.\d+)?)\s*(' + UNIT_PATTERNS['length'] + ')',
        ],
        'feed_opening': [
    ...
    def _extract_manufacturer(self, text: str) -> Optional[str]:
        ...
        for pattern in self.MANUFACTURER_PATTERNS:
            ...
            if match:
                ...
                manufacturer = re.sub(r'\s+(inc|ltd|llc|corporation|corp)\.?$', '', manufacturer, flags=re.IGNORECASE)
    ...
    def _extract_model(self, text: str) -> Optional[str]:
        ...
        for pattern in self.MODEL_PATTERNS:
            match = re.search(pattern, text, re.IGNORECASE)
            if match:
                model = match.group(0).strip() if match.lastindex is None else match.group(1).strip()
                return model.upper()
        return None
    ...
    def _find_parameter(
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                confidence = 0.9 if 'capacity' in pattern.pattern or 'power' in pattern.pattern else 0.8
    ...
    def parse_mill_specifications(self, text: str) -> Dict[str, Any]:
        ...
        dim_pattern = r'(\d+(?:\.\d+)?)\s*(?:m|meters?)\s*x\s*(\d+(?:\.\d+)?)\s*(?:m|meters?)'
    ...
    def parse_crusher_specifications(self, text: str) -> Dict[str, Any]:
        ...
        throw_pattern = r'throw[:\s]+(\d+(?:\.\d+)?)\s*(' + self.UNIT_PATTERNS['length'] + ')'
    ...
    def parse_conveyor_specifications(self, text: str) -> Dict[str, Any]:
        ...
        # Extract belt width
        width_pattern = r'belt\s*width[:\s]+(\d+(?:\.\d+)?)\s*(' + self.UNIT_PATTERNS['length'] + ')'
        ...
        # Extract belt speed
        belt_speed_pattern = r'belt\s*speed[:\s]+(\d+(?:\.\d+)?)\s*(' + self.UNIT_PATTERNS['speed'] + ')'
        ...
        lift_pattern = r'(?:lift|vertical\s*rise)[:\s]+(\d+(?:\.\d+)?)\s*(' + self.UNIT_PATTERNS['length'] + ')'
