...
def _extract_tables_from_pdf(
    ...
    try:
        ...
        logger.warning("[TABLE_EXTRACT] pdf_extract module not found, falling back to Camelot")
        return _extract_tables_camelot(pdf_bytes, pages)
    except Exception as e:
        logger.error(f"[TABLE_EXTRACT] Cascade extraction failed: {e}, falling back to Camelot")
        return _extract_tables_camelot(pdf_bytes, pages)
...
def enhance_with_lineage(
    ...
    Args:
        kg_data: Extracted KG with nodes and edges
        table_meta: Table metadata
        rows: Source row documents

    Returns:
        Enhanced KG data with lineage edges
    """
    table_id = table_meta.get("table_id")

    # Create TableRow nodes if not present
    table_row_nodes = []
    for row in rows:
    ...
    for node in kg_data.get("nodes", []):
        ...
        if row_idx is not None:
            ...
            matching_row = next(
                (r for r in rows if r.get("row_idx") == row_idx),
                None
            )
            if matching_row:
...
@router_ingest_tables.post("/map/tables-to-entities")
def map_tables_to_entities(
    ...
    try:
        ...
        logger.info(f"[TABULAR_PIPELINE] Extracted {len(table_results)} tables from PDF")
    except Exception as e:
    ...
    for table_result in table_results:
        ...
        try:
            ...
            # Call LLM
            logger.info(f"[TABULAR_PIPELINE] Calling LLM for table {table_id} ({len(rows_batch)} rows)")
            kg_data = call_llm_for_tables(input_text, ontology)
            
            # Check for errors in response
            if "error" in kg_data:
                logger.error(f"[TABULAR_PIPELINE] LLM extraction failed for {table_id}: {kg_data['error']}")
            ...
            logger.info(f"[TABULAR_PIPELINE] Table {table_id}: {nodes_written} nodes, {edges_written} edges")
