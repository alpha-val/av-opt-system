...
async def run_analysis(
    ...
    try:
        ...
        logger.info(
            f"Clearing existing data for scenario {scenario_id} before re-running analysis"
        )
                ...
                document_results.append(
                    {
                        ...
                        "entities_for_costing_count": len(
                            result.get("entities_for_costing", [])
                        ),
        ...
        if errors:
            ...
            final_status = (
                ...
                if len(errors) < len(project.base_case_documents)
        ...
        return {
            ...
            "message": (
                "Analysis completed"
                if not errors
                else f"Analysis completed with {len(errors)} error(s)"
            ),
        ...
        logger.error(
            f"Error running analysis for scenario {scenario_id}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def run_analysis_v2(
    ...
    extract_summary: bool = Query(
        False, description="Whether to extract document summary"
    ),
    extraction_scope: str = Query(
        "exact",
        description="Extraction scope: 'exact', 'with_relationships', or 'with_context'",
    ),
    current_user: dict = Depends(get_current_user),
):
    """
    ...
    try:
        ...
        # Clear all data associated with this scenario before re-running analysis
        logger.info(
            f"Clearing existing data for scenario {scenario_id} before re-running analysis (V2)"
        )
        clear_result = await services.clear_data(scenario_id)
        logger.info(f"Cleared scenario data: {clear_result.get('deleted_counts', {})}")
                ...
                document_results.append(
                    {
                        ...
                        "summary_id": result.get("summary_id"),
                        "recommendations_id": result.get("recommendations_id"),
                        "relevant_entities_count": result.get(
                            "relevant_entities_count", 0
                        ),
                        "recommendations_count": len(result.get("recommendations", [])),
                        "extraction_scope": result.get(
                            "extraction_scope", extraction_scope
                        ),
                    }
                )
        ...
        return {
            ...
            "message": (
                "Analysis completed"
                if not errors
                else f"Analysis completed with {len(errors)} error(s)/warning(s)"
            ),
            "documents_processed": len(document_results),
            "document_results": document_results,
            "errors": errors if errors else None,
        ...
        logger.error(
            f"Error running analysis (V2) for scenario {scenario_id}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_scenario_recommendations(
    ...
    try:
        ...
        logger.error(
            f"Error fetching recommendations for scenario {scenario_id}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_scenario_entities(
    ...
    artifact_type: str = Query(
        "base_case", description="Artifact type filter (default: base_case)"
    ),
    current_user: dict = Depends(get_current_user),
):
    """
    ...
    try:
        ...
        logger.error(
            f"Error fetching entities for scenario {scenario_id}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def _run_analysis_background_v3(
    ...
    try:
        ...
        from ....domain.parsing.utils.text_utils import (
            extract_and_clean,
            extract_fulltext,
        )
        ...
        for idx, doc in enumerate(documents, 1):
            ...
            for entity in relevant_entities:
                ...
                logger.info(
                    f"Entity scenario_id: {entity['properties'].get('scenario_id')}"
                )
                logger.info(
                    f"Entity artifact_type: {entity['properties'].get('artifact_type')}"
                )
                logger.info(f"Entity name: {entity['properties'].get('name')}")
                logger.info(
                    f"Entity discipline: {entity['properties'].get('discipline')}"
                )
                logger.info(f"Entity category: {entity['properties'].get('category')}")
                logger.info(
                    f"Entity subcategory: {entity['properties'].get('subcategory')}"
                )
                logger.info(f"Entity entity: {entity['properties'].get('entity')}")
                logger.info(
                    f"Entity extraction_priority: {entity['properties'].get('extraction_priority')}"
                )
                logger.info(
                    f"Entity extraction_rationale: {entity['properties'].get('extraction_rationale')}"
                )
                logger.info(
                    f"Entity expected_attributes: {entity['properties'].get('expected_attributes')}"
                )
                logger.info(f"Entity priority: {entity['properties'].get('priority')}")
                logger.info(
                    f"Entity rationale: {entity['properties'].get('rationale')}"
                )
                logger.info(
                    f"Entity expected_attributes: {entity['properties'].get('expected_attributes')}"
                )
...
async def run_analysis_v3(
    ...
    try:
        ...
        # Clear all data associated with this scenario before re-running analysis
        logger.info(
            f"Clearing existing data for scenario {scenario_id} before re-running analysis (V3)"
        )
        clear_result = await services.clear_data(scenario_id)
        logger.info(f"Cleared scenario data: {clear_result.get('deleted_counts', {})}")
        ...
        logger.error(
            f"Error starting V3 analysis for scenario {scenario_id}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def run_analysis_v4(
    ...
    try:
        ...
        # Validate objective requirements
        if (
            not scenario.global_objective_type
            or not scenario.global_objective_target
            or not scenario.global_objective_unit
        ):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        # Clear all data associated with this scenario before re-running analysis
        logger.info(
            f"Clearing existing data for scenario {scenario_id} before re-running analysis (V4)"
        )
        clear_result = await services.clear_data(scenario_id)
        logger.info(f"Cleared scenario data: {clear_result.get('deleted_counts', {})}")
        ...
        logger.error(
            f"Error starting V4 analysis for scenario {scenario_id}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to start analysis: {str(e)}",
...
async def _run_analysis_background_v4(
    ...
    try:
        ...
        # Publish event after reading documents
        publisher.publish(
            job_id,
            ProgressEvent(
                job_id=job_id,
                ...
                meta={
                    ...
                    "message": "Reading base case documents",
                },
            ),
        )
        seq += 1
        ...
        for idx, doc in enumerate(documents, 1):
            ...
            # Publish event before processing document
            progress_before = 20 + int((idx - 1) * 60 / total_docs)
            publisher.publish(
                job_id,
                ProgressEvent(
                    job_id=job_id,
            ...
            task = _analysis_v4_tasks.get(job_id)
            if task and task.cancelled():
                logger.info(f"Job {job_id} was cancelled, stopping document processing")
            ...
            extraction_result = (
                await orchestration._extract_objective_driven_nodes_and_relations_v4(
                    project_id=project_id,
                    scenario_id=scenario_id,
                    full_text=full_text,
                    global_objective_type=global_objective_type,
                    ...
                    document_id=document_id,
                    job_id=job_id,
                    publisher=publisher,  # Pass publisher instance
                    base_seq=seq,  # Pass base sequence number
                )
            )
            ...
            # Increment seq to account for events published during extraction
            # Extraction publishes: 1 (chunking) + num_chunks (per chunk) + 1 (final) = num_chunks + 2 events
            ...
            # Publish event after processing document
            progress_after = 30 + int(idx * 60 / total_docs)
            publisher.publish(
                job_id,
                ProgressEvent(
                    job_id=job_id,
                    ...
                    meta={
                        ...
                        "message": "Storing entities and edges",
                    },
                ),
            )
            seq += 1
        ...
        # Publish event after storing all entities
        publisher.publish(
            job_id,
            ProgressEvent(
                job_id=job_id,
        ...
        logger.info(f"V4 analysis job {job_id} was cancelled")
        ...
        # Update scenario status to cancelled
        try:
            await services.update(
                scenario_id,
                ScenarioUpdate(status=ScenarioStatus.CANCELLED),
            )
        except Exception as e:
            logger.warning(f"Failed to update scenario status after cancellation: {e}")
...
@scenarios_router.post(
    ...
    description="Cancel a running V4 analysis job by job_id.",
)
async def cancel_analysis_v4(
    scenario_id: str = Path(
        ..., description="Scenario ID (MongoDB ObjectId as string)"
    ),
    ...
    try:
        ...
        if job_id not in _analysis_v4_tasks:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Job {job_id} not found or already completed",
            )

        task = _analysis_v4_tasks[job_id]
        ...
        if cancelled:
            logger.info(f"Cancelled V4 analysis job {job_id} for scenario {scenario_id}")
            ...
            try:
                ...
                publisher.publish(
                    ...
                    ProgressEvent(
                        ...
                        status=Status.FAILED,
                        progress=0,
                        seq=999999,  # High seq to ensure it's the last event
                        meta={
                            "scenario_id": scenario_id,
                            "message": "Analysis cancelled by user",
                        },
            ...
            return {
                ...
                "scenario_id": scenario_id,
                "status": "cancelled",
                "message": f"Analysis job {job_id} has been cancelled",
            }
        else:
            # Task couldn't be cancelled (might be too late)
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Could not cancel job {job_id} (may have already started final operations)",
            )
        ...
        logger.error(
            f"Error cancelling V4 analysis job {job_id} for scenario {scenario_id}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
