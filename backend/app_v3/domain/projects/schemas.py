...
class ProjectBase(BaseModel):
    ...
    objective_description: Optional[str] = Field(
        None, 
        max_length=500, 
        description="Additional context on global objective"
    )
    # Project status
    ...
    base_case_documents: List[str] = Field(
        ...
        description="List of document IDs for base case reports"
    )
...
class ProjectUpdate(BaseModel):
    ...
    description: Optional[str] = Field(None, max_length=500, description="Project description")
    tags: Optional[List[str]] = Field(None, description="List of tags for categorization")
    global_objective_type: Optional[str] = Field(None, description="Type of global objective")
    global_objective_target: Optional[str] = Field(None, description="Target magnitude of change")
    objective_description: Optional[str] = Field(None, max_length=500, description="Additional context on global objective")
    status: Optional[ProjectStatus] = Field(None, description="Current status of the project")
    base_case_documents: Optional[List[str]] = Field(None, description="List of document IDs for base case reports")
    tabular_data_documents: Optional[List[str]] = Field(None, description="List of document IDs for tabular data")
...
class ProjectOut(ProjectBase):
    ...
    """
    id: str = Field(..., description="Unique project identifier (MongoDB ObjectId as string)")
