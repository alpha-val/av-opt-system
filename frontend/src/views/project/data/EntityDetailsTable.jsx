    ...
    const filteredEntities = useMemo(() => {
        ...
        if (selectedEntityType !== 'all') {
            filtered = filtered.filter(
                (entity) =>
                    (entity.type || entity.entity_type || 'unknown') === selectedEntityType
            );
        }
        ...
        if (searchTerm) {
            ...
            filtered = filtered.filter((entity) => {
                const name = (entity.properties?.name || entity.name || '').toLowerCase();
                const type = (entity.type || entity.entity_type || '').toLowerCase();
                const description = (entity.properties?.description || '').toLowerCase();
                
                // Search in properties
                const propertiesString = JSON.stringify(entity.properties || {}).toLowerCase();
